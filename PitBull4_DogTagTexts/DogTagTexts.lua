...
function PitBull4_DogTagTexts:OnNewLayout(layout)
	...
	for name, data in pairs {
		...
		[L["Mana"]] = {
			code = "[if HasMana then FractionalMana]",
			attach_to = "ManaBar",
			location = "right",
		},
		[L["Rage"]] = {
			code = "[if HasRage then Rage]",
			attach_to = "RageBar",
			location = "right",
		},
		[L["Energy"]] = {
			code = "[if HasEnergy then Energy]",
			attach_to = "EnergyBar",
			location = "right",
		},
		[L["Reputation"]] = {
...
local PROVIDED_CODES = function() return {
	...
	[L["Mana"]] = {
		[L["Absolute"]]       = "[if HasMana then FractionalMana]",
		[L["Absolute short"]] = "[if HasMana then FractionalMana:Short]",
		[L["Difference"]]     = "[-MissingMana]",
		[L["Percent"]]        = "[PercentMana:Percent]",
		[L["Absolute and percent"]]  = "[if HasMana then FractionalMana] || [PercentMana:Percent]",
		[L["Mini"]]           = "[if HasMana then Mana:VeryShort]",
		[L["Smart"]]          = "[MissingMana:Hide(0):Short:Color('7f7fff')]",
	},
	[L["Rage"]] = {
		[L["Absolute"]]       = "[if HasRage then FractionalRage]",
		[L["Absolute short"]] = "[if HasRage then FractionalRage:Short]",
		[L["Difference"]]     = "[-MissingRage]",
		[L["Percent"]]        = "[PercentRage:Percent]",
		[L["Absolute and percent"]]  = "[if HasRage then FractionalRage] || [PercentRage:Percent]",
		[L["Mini"]]           = "[if HasRage then Rage:VeryShort]",
		[L["Smart"]]          = "[MissingRage:Hide(0):Short:Color('7f7fff')]",
	},
	[L["Energy"]] = {
		[L["Absolute"]]       = "[if HasEnergy then FractionalEnergy]",
		[L["Absolute short"]] = "[if HasEnergy then FractionalEnergy:Short]",
		[L["Difference"]]     = "[-MissingEnergy]",
		[L["Percent"]]        = "[PercentEnergy:Percent]",
		[L["Absolute and percent"]]  = "[if HasEnergy then FractionalEnergy] || [PercentEnergy:Percent]",
		[L["Mini"]]           = "[if HasEnergy then Energy:VeryShort]",
		[L["Smart"]]          = "[MissingEnergy:Hide(0):Short:Color('7f7fff')]",
	},
	[L["Druid mana"]] = {
